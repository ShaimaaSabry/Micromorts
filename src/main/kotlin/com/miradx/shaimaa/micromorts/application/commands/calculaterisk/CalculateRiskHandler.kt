...
import com.miradx.shaimaa.micromorts.domain.exception.CalculateRiskException
...
class CalculateRiskHandler(
    ...
    operator fun invoke(command: CalculateRiskCommand): CalculateRiskResult {
        val actions = command.actions.map {
            Action.create(
                it.timestamp,
                it.action,
                it.unit,
        ...
        val activities = this.activityRepository.searchActivities(
        ...
        for (action in actions) {
            val activity = activities[action.action]?.orElseThrow {
